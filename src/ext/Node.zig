        ...
        while (batches.next()) |batch| {
            switch (batch) {
                .arch_change => |arch_change| {
                    var iter = arch_change.iterator();
                    while (iter.next()) |op| {
                        beforeArchChangeImmediate(es, arch_change, op);
                    }

                    // XXX: i guess it was nice that we could accumulate the arch change so that we're
                    // not iterating so many extra times, we can add that back if we really want once we
                    // get things working
                    _ = try arch_change.execImmediateOrErr(es);
                },
                .ext => |ext| if (ext.as(SetParent)) |set_parent| {
