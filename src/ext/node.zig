...
pub fn NodeWithOptions(options: Options) type {
    return struct {
        const Node = @This();

        /// The node's name.
        ///
        /// Ignored internally. Simplest use case is a static string, if you want to dynamically
        /// allocate these you'll need to hook into the command buffer to free them
        /// when the node is destroyed. This could have be
        ///
        /// This could have been implemented as a separate component, but adding it here reduces
        /// fragmentation.
        name: options.Name = std.mem.zeroes(options.Name),
        /// This node's parent.
        ...
        ///
        /// Node iterators for example need to retrieve both the node and the entity to function,
        /// and the caller may be interested in either or both.
        ...
        /// Initializes this node. Called automatically by the command buffer API, must be called
        /// manually before using a node if working with the immediate API.
        ...
        /// Updates `active_in_hierarchy` for this node, and propagates any resulting change to its
        /// transitive children.
        ...
        /// Returns a pointer to the `T` component of the nearest ancestor containing `T`, or `null`
        /// if no ancestors contain a `T` component.
        ...
        pub fn setParentImmediate(self: *Node, es: *Entities, tr: *Tree, parent_opt: ?*Node) void {
            ...
            // If the relationship would result in a cycle, or parent and child are equal, early
            // out.
        ...
        /// Destroys a node, its entity, and all of its children. This behavior occurs automatically
        /// via `exec` when an entity with an entity with a node is destroyed.
        ///
        ...
        /// Intended for internal use, but may have advanced external uses. Removes this node from
        /// its parents without adding it to the root of the tree. To result in a well formed tree,
        /// you must follow this up by destroying the entity, removing the node, or replacing it
        ...
        /// Intended for internal use, see `pluckImmediate`. Similar but destroys children before
        /// plucking.
        ///
        /// Invalidates pointers.
        ...
        /// Returns true if an ancestor of `descendant`, false otherwise. Entities are not ancestors
        /// of themselves.
        ...
        /// Returns an iterator over the node's ancestors. The iterator starts at the parent, if
        /// any, and then, follows the parent chain until it hits a node with no parent.
        ...
        /// Returns a pre-order iterator over a node's children. Pre-order traversal visits parents
        /// before children.
        ...
        pub const PreOrderIterator = struct {
            ...
            pub fn skipSubtree(self: *@This(), es: *const Entities, subtree: *const Node) void {
                // Assert that subtree is contained by this iterator. If it isn't, we'd end up with
                // an infinite loop.
        ...
        /// Returns a post-order iterator over a node's children. Post-order traversal visits
        ...
        ///
        /// By convention, `exec` only calls into the stable public interface of the types it's
        /// working with. As such, documentation is sparse. You are welcome to call these methods
        /// directly, or use them as reference for implementing your own command buffer iterator.
        pub const Exec = struct {
            /// Provided as reference. Executes a command buffer, maintaining the hierarchy and
            /// reacting to related events along the way. In practice, you likely want to call the
            /// finer grained functions provided directly, so that other libraries you use can also
            /// hook into the command buffer iterator.
            pub fn immediate(es: *Entities, cb: *CmdBuf, tr: *Tree) void {
                immediateOrErr(es, cb, tr) catch |err|
                    @panic(@errorName(err));
            }

            /// Similar to `immediate`, but returns an error on failure instead of panicking. On
            /// error the commands are left partially evaluated.
            ///
            /// Invalidates pointers.
