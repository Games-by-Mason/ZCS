...
const FuzzCmdBuf = struct {
    ...
    fn expectedOfArchetype(self: *@This(), archetype: Component.Flags) usize {
        ...
        while (iter.next()) |entry| {
            if (archetype.contains(self.es.comp_types.registerIndex(RigidBody))) {
                if (entry.value_ptr.rb == null) continue;
            }
            if (archetype.contains(self.es.comp_types.registerIndex(Model))) {
                if (entry.value_ptr.model == null) continue;
            }
            if (archetype.contains(self.es.comp_types.registerIndex(Tag))) {
